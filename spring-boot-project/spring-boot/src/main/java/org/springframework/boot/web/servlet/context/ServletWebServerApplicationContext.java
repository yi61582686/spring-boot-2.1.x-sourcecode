...
public class ServletWebServerApplicationContext extends GenericWebApplicationContext
	...
	private void createWebServer() {
		// 获取webServer和servletContext，来判断当前Context中是否经初始化了Server
		WebServer webServer = this.webServer;
		ServletContext servletContext = getServletContext();
		if (webServer == null && servletContext == null) { // 第一次启动会进入if判断
			// 获取Server工厂：TomcatServletWebServerFactory
			ServletWebServerFactory factory = getWebServerFactory();
			/**
			 * getSelfInitializer()方法：ServletContextInitializer
			 * 创建tomcat，并启动
			 *
			 */
			this.webServer = factory.getWebServer(getSelfInitializer());
		}
		else if (servletContext != null) { // 第一次不会进入这里
			try {
				// 已经有WebServer，启动服务器
	...
	private org.springframework.boot.web.servlet.ServletContextInitializer getSelfInitializer() {
		// ::语法。
		// this::selfInitialize 等价于 this.selfInitialize
		return this::selfInitialize;
	}

	private void selfInitialize(ServletContext servletContext) throws ServletException {
		// 准备SpringMVCApplicationContext
		prepareWebApplicationContext(servletContext);
		registerApplicationScope(servletContext);
		WebApplicationContextUtils.registerEnvironmentBeans(getBeanFactory(), servletContext);
		// getServletContextInitializerBeans()：获取到Web环境初始化时需要的组件：Servlet，Filter，Listener等
